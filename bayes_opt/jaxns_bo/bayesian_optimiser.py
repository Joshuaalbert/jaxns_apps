...
from jaxns_bo.utils import latin_hypercube
from jaxns_gp.kernels import RBF
...
class BayesianOptimiser(object):
    def __init__(self, model: Model, key=None, load_file: str = None,
    ...
    def posterior_solve(self, key, U, Y):
        ...
        def prior_model() -> PriorModelGen:
            x = yield Prior(tfpd.Laplace(loc=0., scale=1.))
            lengthscale = yield Prior(jnp.abs(x), name='lengthscale')
            x = yield Prior(tfpd.Laplace(loc=0., scale=0.1))
            uncert = yield Prior(jnp.abs(x), name='uncert')
            x = yield Prior(tfpd.Laplace(loc=0., scale=1.))
            sigma = yield Prior(jnp.abs(x), name='sigma')
            return sigma, lengthscale, uncert
        ...
        ns = ExactNestedSampler(model=model, num_live_points=100,
